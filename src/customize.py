...
class customize(Callback):
    # 自定义的keras回调
    def __init__(self,savedir,metrics):
    ...
    def on_epoch_end(self,epoch,logs={}):
        ...
        for metric in self.metrics:
            ...
            self.group1['{}-epoch'.format(epoch+1)].create_dataset(metric,data=self.eva_dict[metric])
            self.group2['{}-epoch'.format(epoch+1)].create_dataset(metric,data=self.val_eva_dict[metric])
        self.file.close()
